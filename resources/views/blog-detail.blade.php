@extends('layouts.app')

@section('title', $article->title . ' - ARWebStudio')
@section('meta_description', $article->excerpt ?? Str::limit(strip_tags($article->content), 155))
@section('canonical', route('blog.show', $article->slug))

{{-- Open Graph per artikel --}}
@section('og_title', $article->title . ' | ARWebStudio')
@section('og_description', $article->excerpt ?? Str::limit(strip_tags($article->content), 155))
@section('og_image', $article->thumbnail ? asset('storage/' . $article->thumbnail) : asset('images/og-image.jpg'))
@section('twitter_title', $article->title . ' | ARWebStudio')
@section('twitter_description', $article->excerpt ?? Str::limit(strip_tags($article->content), 155))
@section('twitter_image', $article->thumbnail ? asset('storage/' . $article->thumbnail) : asset('images/og-image.jpg'))

@section('content')
<div class="flex flex-col w-full bg-[#101415]">

    <!-- ===== BACK BUTTON ===== -->
    <div class="max-w-[1280px] mx-auto px-5 lg:px-16 w-full pt-8">
        <a href="{{ route('blog') }}" 
           class="inline-flex items-center gap-2 text-[#c5c6ce] hover:text-[#F5A623] transition-colors group">
            <span class="material-symbols-outlined text-sm group-hover:-translate-x-1 transition-transform">arrow_back</span>
            <span class="text-sm font-semibold uppercase tracking-widest">Kembali ke Blog</span>
        </a>
    </div>

    <!-- ===== ARTICLE ===== -->
    <article class="px-5 lg:px-16 py-12 lg:py-20">
        <div class="max-w-[900px] mx-auto">
            
            <!-- Category & Date -->
            <div class="flex flex-wrap items-center gap-3 mb-4">
                @if($article->category)
                    <span class="bg-[#F5A623]/10 text-[#F5A623] text-xs font-semibold px-3 py-1 rounded-full border border-[#F5A623]/30">
                        {{ $article->category }}
                    </span>
                @endif
                <span class="text-sm text-[#c5c6ce]">{{ $article->formatted_date }}</span>
                <span class="text-sm text-[#c5c6ce]">•</span>
                <span class="text-sm text-[#c5c6ce]">{{ $article->reading_time }}</span>
            </div>

            <!-- Title -->
            <h1 class="font-['Sora'] text-3xl lg:text-5xl font-bold text-[#e0e3e5] leading-tight">
                {{ $article->title }}
            </h1>

            <!-- Author -->
            <div class="flex items-center gap-3 mt-4 pb-6 border-b border-[rgba(74,127,199,0.1)]">
                <div class="w-10 h-10 rounded-full bg-[#114784] flex items-center justify-center text-[#d5e3ff] font-semibold">
                    {{ substr($article->author ?? 'AR', 0, 2) }}
                </div>
                <div>
                    <p class="text-sm font-semibold text-[#e0e3e5]">{{ $article->author ?? 'ARWebStudio' }}</p>
                    <p class="text-xs text-[#c5c6ce]">Penulis</p>
                </div>
            </div>

            <!-- Thumbnail -->
            @if($article->thumbnail)
                <div class="mt-6 rounded-2xl overflow-hidden">
                    <img src="{{ asset('storage/' . $article->thumbnail) }}" 
                         alt="{{ $article->title }}" 
                         fetchpriority="high"
                         class="w-full h-auto object-cover">
                </div>
            @endif

            <!-- Content -->
            <div class="mt-8 article-content">
                {!! sanitize_html($article->content) !!}
            </div>

            <!-- Share -->
            <div class="mt-12 pt-6 border-t border-[rgba(74,127,199,0.1)]">
                <p class="text-sm text-[#c5c6ce] mb-4">Bagikan artikel ini:</p>
                <div class="flex gap-3">
                    <a href="https://wa.me/?text={{ urlencode($article->title . ' - ' . route('blog.show', $article->slug)) }}" 
                       target="_blank"
                       class="w-10 h-10 rounded-full bg-[#25D366] flex items-center justify-center text-white hover:scale-110 transition-transform">
                        <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    </a>
                    <a href="https://twitter.com/intent/tweet?text={{ urlencode($article->title) }}&url={{ urlencode(route('blog.show', $article->slug)) }}" 
                       target="_blank"
                       class="w-10 h-10 rounded-full bg-[#1DA1F2] flex items-center justify-center text-white hover:scale-110 transition-transform">
                        <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    </a>
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('blog.show', $article->slug)) }}" 
                       target="_blank"
                       class="w-10 h-10 rounded-full bg-[#1877F2] flex items-center justify-center text-white hover:scale-110 transition-transform">
                        <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                    <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(route('blog.show', $article->slug)) }}" 
                       target="_blank"
                       class="w-10 h-10 rounded-full bg-[#0A66C2] flex items-center justify-center text-white hover:scale-110 transition-transform">
                        <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                    </a>
                    
                    <button onclick="copyLink()" 
                            class="w-10 h-10 rounded-full bg-[#c5c6ce]/20 flex items-center justify-center text-[#e0e3e5] hover:bg-[#c5c6ce]/40 hover:scale-110 transition-all">
                        <span class="material-symbols-outlined text-sm">content_copy</span>
                    </button>

                    <script>
                    function copyLink() {
                        navigator.clipboard.writeText('{{ route('blog.show', $article->slug) }}');
                        alert('Link artikel berhasil disalin!');
                    }
                    </script>
                </div>
            </div>
        </div>
    </article>

    <!-- ===== RELATED ARTICLES ===== -->
    @if($recentArticles->count() > 0)
    <section class="px-5 lg:px-16 pb-24">
        <div class="max-w-[1280px] mx-auto">
            <div class="flex items-center justify-between mb-8">
                <h2 class="font-['Sora'] text-2xl lg:text-3xl font-semibold text-[#e0e3e5]">Artikel Terkait</h2>
                <a href="{{ route('blog') }}" class="text-[#F5A623] text-sm font-semibold hover:underline">Lihat Semua</a>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach($recentArticles as $related)
                    <a href="{{ route('blog.show', $related->slug) }}" 
                       class="group bg-[#191c1e] rounded-xl overflow-hidden border border-[rgba(74,127,199,0.1)] hover:border-[#F5A623]/30 transition-all duration-300 hover:-translate-y-1">
                        <div class="aspect-[16/9] overflow-hidden bg-[#1d2022]">
                            @if($related->thumbnail)
                                <img src="{{ asset('storage/' . $related->thumbnail) }}" 
                                     alt="{{ $related->title }}" 
                                     class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            @else
                                <div class="w-full h-full flex items-center justify-center text-[#a8c8ff]/30">
                                    <span class="material-symbols-outlined text-4xl">article</span>
                                </div>
                            @endif
                        </div>
                        <div class="p-4">
                            <h3 class="font-['Sora'] text-base font-semibold text-[#e0e3e5] group-hover:text-[#F5A623] transition-colors line-clamp-2">
                                {{ $related->title }}
                            </h3>
                            <p class="text-[#c5c6ce] text-xs mt-1">{{ $related->formatted_date }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
        </div>
    </section>
    @endif

    <!-- ===== CTA ===== -->
    <section class="px-5 lg:px-16 pb-24">
        <div class="max-w-[1280px] mx-auto bg-gradient-to-br from-[#191c1e] via-[#1d2022] to-[#272a2c] rounded-[2rem] p-8 lg:py-16 lg:px-16 flex flex-col items-center text-center gap-6 border border-white/[0.05]">
            <h2 class="font-['Sora'] text-2xl lg:text-4xl font-semibold text-[#e0e3e5]">Butuh Bantuan Untuk Website Anda?</h2>
            <p class="text-[#c5c6ce] max-w-xl">Konsultasikan kebutuhan website Anda dengan tim ahli kami secara gratis.</p>
            <a href="{{ route('contact') }}" class="bg-[#F5A623] text-[#0d1b35] px-10 py-4 rounded-full font-semibold hover:scale-105 transition-all">
                Hubungi Kami
            </a>
        </div>
    </section>
</div>
@endsection

@push('structured_data')
<script type="application/ld+json">
{
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": "{{ addslashes($article->title) }}",
    "description": "{{ addslashes($article->excerpt ?? Str::limit(strip_tags($article->content), 155)) }}",
    "datePublished": "{{ $article->published_at ? $article->published_at->toIso8601String() : $article->created_at->toIso8601String() }}",
    "dateModified": "{{ $article->updated_at->toIso8601String() }}",
    "author": {
        "@type": "Organization",
        "name": "ARWebStudio",
        "url": "{{ url('/') }}"
    },
    "publisher": {
        "@type": "Organization",
        "name": "ARWebStudio",
        "logo": {
            "@type": "ImageObject",
            "url": "{{ asset('images/logo-arwebstudio.png') }}"
        }
    },
    "url": "{{ route('blog.show', $article->slug) }}",
    @if($article->thumbnail)
    "image": "{{ asset('storage/' . $article->thumbnail) }}",
    @endif
    "mainEntityOfPage": {
        "@type": "WebPage",
        "@id": "{{ route('blog.show', $article->slug) }}"
    }
}
</script>
@endpush